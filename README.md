...
- [聚合类型与静态容器（讨论稿）](docs/语法设计/聚合类型与静态容器（讨论稿）.md)：元组、`T[]`、`List<T>`、`Set<T>` 的字面量、元素访问、遍历和当前静态布局边界。
...
动态数组/泛型容器、继承与闭包、用户并发、跨重新部署持久化，以及完整 profile 指令签名表仍在后续范围。当前已支持静态布局的元组、数组、List 与 Set；动态长度、嵌套容器和运行时下标仍等待 Memory runtime。包解析、静态函数 ABI、String/对象的资源上界和基础 I/O 语义已有设计约定。欢迎通过 Issue 或 Pull Request 参与；提交涉及语法变化时，请同步更新语言规范中的“讨论记录”。

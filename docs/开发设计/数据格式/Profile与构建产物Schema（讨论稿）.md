...
多 shard 构建还包含 `runtimeTopology.sharedRuntime`：记录 magic、ABI、布局指纹、部署 epoch、Main/Worker 身份、ready/回执下标、header 的物理 slice，以及每个 SPSC 邮箱的生产者、消费者、payload 宽度和 slice。该字段来自实际代码生成使用的同一份布局，不允许由报告层重新估算；单 shard 构建省略它。

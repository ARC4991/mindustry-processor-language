...
export const mplLessons: TutorialLesson[] = [
  ...
  },
  {
    ...
    sections: [
      { title: "返回类型也能推导", paragraphs: ["函数参数必须标注类型；返回类型可从所有 return 路径推导。函数和类支持后置声明，调用点不受源码排列顺序限制。顶层函数和对象方法都可传入并返回数值/Bool 元组与固定形状数组。"], terms: ["fun", "return", "返回类型", "后置声明", "Int[]", "元组", "数组"], code: { language: "mpl", source: "val rotated = rotate([1, 2, 3]); // Int[]\n\nfun rotate(values: Int[]) {\n  return [values[2], values[0], values[1]];\n}" } },
